...
class MemberStockServiceUtils {
    ...
    static void validateNotExistStockInMember(MemberStockRepository memberStockRepository, Long stockId, Long memberId) {
        ...
        if (memberStock != null) {
            throw new ConflictException(String.format("멤버 (%s)는 해당하는 주식 (%s)을 이미 소유하고 있습니다", memberId, stockId), ErrorCode.CONFLICT_MEMBER_STOCK_EXCEPTION);
        }
    }
    ...
    static MemberStock findMemberStockByIdAndMemberId(MemberStockRepository memberStockRepository, Long memberStockId, Long memberId) {
        ...
        if (memberStock == null) {
            throw new NotFoundException(String.format("멤버 (%s)에게 해당하는 소유 주식 (%s)이 존재하지 않습니다", memberId, memberStockId), ErrorCode.NOT_FOUND_STOCK_EXCEPTION);
        }
        return memberStock;
    }
